...
export default function PropertyAddressEdit (props:any)  {
   // const [street_address, setAddress] = useState<any>(null);
    ...
    const { street_address, postcode, lat,long } = data;
    ...
    function getAddressByCoord(lat:number,lng:number) {
        setData({...data,'lats':lat});
        axios({
            method: 'get', //you can set what request you want to be
            url: API.GeocodingUrl+lat+','+lng +'&key='+API.API_KEY,
        }).then((res:any)=>{
            try {
                setData({...data,[postcode]:res.data.results[0].address_components.find((addr:any) => addr.types[0] === "postal_code").short_name});
            ...
            try {
                setData({ ...data, street_address:res.data.results[0].formatted_address});
                setShowLoading(false);
    ...
    function nav(e:any) {
        ...
        updateData(data,'properties',localStorage.getItem('property_id'))
            .then(res=>{
                setShowLoading(false);
            ...
            <IonContent fullscreen>
                ...
                <div className={'ion-margin-top ion-padding'}>
                    ...
                    <div className='ion-margin-top ion-text-center'>
                        <IonText  color='primary'>{street_address}</IonText>
                    </div>
                </div>
                ...
                <form onSubmit={event1 => nav(event1)}>
                    <div>
                        <IonItem color={'none'} lines={'none'}>
                            ...
                            <IonInput name='postcode'  value={postcode} onIonChange={e => onInputChange(e)}  className={'input'}  />
                            <IonInput hidden name='lat'  value={lat} onIonChange={e => onInputChange(e)}  className={'input'}  />
                            <IonInput hidden name='long'  value={long} onIonChange={e => onInputChange(e)}  className={'input'}  />
                        </IonItem>
                    </div>


                    <div hidden className={'ion-padding'}>
                    ...
                    <div >
                        <IonItem color={'none'} lines={'none'}>
                            <IonLabel hidden position={'stacked'} >Select address</IonLabel>
                            {/*<IonSelect disabled={!addresses} value={data?.street_address} className={'input-select'} onIonChange={e => onInputChange(e)} >*/}
                        ...
                        <IonItem color={'none'} lines={'none'}>
                            ...
                            <IonInput name="street_address" required value={street_address} onIonChange={e => onInputChange(e)} className={'input'}  />
                        </IonItem>
                    </div>
